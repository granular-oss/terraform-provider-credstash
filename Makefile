...
run_integration_test_bash: install
	...
	@credstash delete terraform-provider-credstash-integration-test-8
	...
	@credstash put -a terraform-provider-credstash-integration-test-8 "test-8-1"
	@credstash put -a terraform-provider-credstash-integration-test-8 "test-8-2"
	...
	@cd integration_test/tf; TF_LOG=$(TF_LOG) TF_VAR_secret_1_version=1 TF_VAR_secret_1=IAmABadPassword1 TF_VAR_secret_3=IAmABadPassword2 TF_VAR_secret_5=test-5-2 terraform import credstash_secret.terraform-provider-credstash-integration-test-8 terraform-provider-credstash-integration-test-8
...
## Import Generate Tests
# Test terraform-provider-credstash-integration-test-8 Imported Value Matches credstash latest version
	@T_VALUE=$$(cd integration_test/tf; terraform show -json | jq --raw-output '.values.root_module.resources[] | select(.address | contains("terraform-provider-credstash-integration-test-8")).values.value') E_VALUE=$$(credstash get terraform-provider-credstash-integration-test-8 --version 2 -n) bash -c 'if [ "$$T_VALUE" = "$$E_VALUE" ]; then echo -e "\033[0;32mValue Correct For terraform-provider-credstash-integration-test-8 \033[0m"; else echo -e "\033[0;31mValue Incorrect For terraform-provider-credstash-integration-test-8 Got: $${T_VALUE} Expected: $${E_VALUE}\033[0m"; exit 1; fi'
# Test terraform-provider-credstash-integration-test-8 Imported Version is 0, so we can autoincrement in the future
	@T_VALUE=$$(cd integration_test/tf; terraform show -json | jq --raw-output '.values.root_module.resources[] | select(.address | contains("terraform-provider-credstash-integration-test-8")).values.version') E_VALUE=0 bash -c 'if [ "$$T_VALUE" = "$$E_VALUE" ]; then echo -e "\033[0;32mVersion Correct For terraform-provider-credstash-integration-test-8 \033[0m"; else echo -e "\033[0;31mVersion Incorrect For terraform-provider-credstash-integration-test-8 Got: $${T_VALUE} Expected: $${E_VALUE}\033[0m"; exit 1; fi'
# Test terraform-provider-credstash-integration-test-8 Imported Name is correct
	@T_VALUE=$$(cd integration_test/tf; terraform show -json | jq --raw-output '.values.root_module.resources[] | select(.address | contains("terraform-provider-credstash-integration-test-8")).values.name') E_VALUE=terraform-provider-credstash-integration-test-8 bash -c 'if [ "$$T_VALUE" = "$$E_VALUE" ]; then echo -e "\033[0;32mName Correct For terraform-provider-credstash-integration-test-8 \033[0m"; else echo -e "\033[0;31mName Incorrect For terraform-provider-credstash-integration-test-8 Got: $${T_VALUE} Expected: $${E_VALUE}\033[0m"; exit 1; fi'
# TODO: Test terraform-provider-credstash-integration-test-8 Import value didn't change after terraform apply
	@T_VALUE=$$(credstash get terraform-provider-credstash-integration-test-8) E_VALUE="test-8-2" bash -c 'if [ "$$E_VALUE" = "$$T_VALUE" ]; then echo -e "\033[0;32mVersion Correct For terraform-provider-credstash-integration-test-8 \033[0m"; else echo -e "\033[0;31mVersion Incorrect For terraform-provider-credstash-integration-test-8 Got: $${T_VALUE} Expected: $${E_VALUE}\033[0m"; exit 1; fi'
